...
<AccordionGroup>
  <Accordion title="Scary-looking `psycopg2` error">
    ...
    You can install it by running: ```bash brew install postgresql ``` After
    installation, verify that the PostgreSQL path is added to your `PATH`
    environment variable to avoid issues. You can check this by running: ```bash
    echo $PATH ``` If the path is missing, you can add PostgreSQL to your path
    by appending it directly to your shell configuration file (like `.zshrc`,
    `.bash_profile`, or `.bashrc`). For example: ```bash zsh echo 'export
    PATH="/usr/local/opt/postgresql/bin:$PATH"' >> ~/.zshrc ``` Remember to
    source the file afterward to apply the changes immediately: ```bash zsh
    source ~/.zshrc ```
  </Accordion>
